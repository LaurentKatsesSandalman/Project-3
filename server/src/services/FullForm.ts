...
if(initialForm){
    ...
    if(initialForm.is_deployed!== form.is_deployed) {updatedForm.is_deployed=form.is_deployed}
    if(initialForm.is_closed !== form.is_closed) {updatedForm.is_closed=form.is_closed}
    if(initialForm.is_public !== form.is_public) {updatedForm.is_public=form.is_public}
    if(initialForm.multi_answer !== form.multi_answer) {updatedForm.multi_answer=form.multi_answer}
    if(initialForm.theme_id !== form.theme_id) {updatedForm.theme_id=form.theme_id}
    if(initialForm.form_name !== form.form_name) {updatedForm.form_name=form.form_name}
    if(initialForm.form_description !== form.form_description) {updatedForm.form_description=form.form_description}
    if(initialForm.date_to_close !== form.date_to_close) {updatedForm.date_to_close=form.date_to_close}
